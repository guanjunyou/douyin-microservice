...
func (relationServiceImpl RelationServiceImpl) GetFollows(userId int64) ([]models.User, error) {
	...
	err := utils.GetMysqlDB().Table("follow").Where("user_id = ? AND is_deleted != ?", userId, 1).Find(&follows).Error
	if err != nil {
		return nil, err
	}
	...
	for i := 0; i < len(follows); i++ {
		...
		go func(i int) {
			defer wg.Done()
			user, _ := rpc.GetUserById(follows[i].FollowUserId)
			user.IsFollow = true
...
func (relationServiceImpl RelationServiceImpl) GetFollowers(userId int64) ([]models.User, error) {
	...
	var users []models.User
	var follows []models.Follow
	err := utils.GetMysqlDB().Table("follow").Where("follow_user_id = ? AND is_deleted != ?", userId, 1).Find(&follows).Error
	if err != nil {
		return nil, err
	}
	for i := range follows {
		user, _ := rpc.GetUserById(follows[i].UserId)
		users = append(users, *user)
	}
	return users, nil
}
